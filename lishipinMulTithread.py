...
# 根据url获取视频数据并保存
def getAndSaveVideo(url):
    title = url.split('/')[-1]
    print(title + ' downloading...')

     # 获取视频的内容
    video = requests.get(url=url, headers=hd).content
...
if __name__ == '__main__':
    ...
    for videoId in videoIdList:
        ...
        videoUrlList.append(newVideoUrl)
    ...
    # 多线程处理 
    pool = Pool(len(videoUrlList))
    pool.map(getAndSaveVideo, videoUrlList)
    pool.close()
    pool.join()